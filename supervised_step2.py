# Main file to check the performance of the step 2 classifier, the functions are similar as described in semisupervised.py
...
def newlabels(anomalies,semilabels_path):
  # print len(anomalies[anomalies[2] != ' Normal_train']), len(oldlabels)
	...
	for j in range(0,len(labels)):
		if(last_index != -1 and labels[last_index] == 0 and labels[j] == 1):
			if(datetime.strptime(anomalies[0][j],'%Y-%m-%d')-datetime.strptime(anomalies[1][last_index],'%Y-%m-%d') <= timedelta(days=30)):
				newlabels.append(2)
		elif(labels[j]==1):
			newlabels.append(1)
		elif(labels[j] == 0):
...
def prepare(anomalies,labels,priceserieslist):
	...
	for i in range(0,len(anomalies)):
		...
		for j in range(0,len(priceserieslist)):
			start = anomalies[0][i]
			end = datetime.strptime(anomalies[0][i],'%Y-%m-%d') + timedelta(days = 35)
...
def train_test_function(align_m,align_d,align_l,align_b,data_m,data_d,data_l,data_b):
	...
	delhilabelsnew = newlabels(anomaliesdelhi,"data/anomaly/step2_labels_delhi.csv")
	lucknowlabelsnew = newlabels(anomalieslucknow,"data/anomaly/step2_labels_lucknow.csv")
	mumbailabelsnew = newlabels(anomaliesmumbai,"data/anomaly/step2_labels_mumbai.csv")
	bangalorelabelsnew = newlabels(anomaliesbangalore,"data/anomaly/step2_labels_bangalore.csv")
	...
	# for h in range(0,len(y1)):
	# 	print y1[h]
	 

	for y in range(0,len(yall)):
		if( yall[y] == 1 or yall[y]==0):
