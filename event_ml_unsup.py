...
# 		anomalies[0][i] = datetime.strptime(mid_date_index,'%Y-%m-%d')-timedelta(days=21)
# 		anomalies[1][i] = datetime.strptime(mid_date_index,'%Y-%m-%d')+timedelta(days=21)
...
print len(retailpriceseriesdelhi),len(mandiarrivalseriesdelhi),len(mandipriceseriesdelhi)
...
for i in xall:
  print len(i)
scaler = preprocessing.StandardScaler().fit(xall.tolist())
