...
def extract_rainfall_data():
	...
	file_fuel = open('fuel_monthly.csv','w')
# filing the form for times of india archive
	...
	url = 'https://iocl.com/Products/PetrolDomesticPrices.aspx'
	browser.get(url)
	# months = months1
	# for month in months:
	st="Month,Delhi,Kolkata,Mumbai,Chennai\n"
	...
	table = browser.find_element_by_xpath("//*[@id=\"objContPreviousPrices_grdPreviousPrices\"]")
	...
	for row in rows:
		...
		st+=cell+","+cell1.text+","+cell2.text+","+cell3.text+","+cell4.text+"\n"
		count = count + 1
	file_fuel.write(st)
