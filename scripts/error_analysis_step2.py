# This code plots the correct predictions for step 2 for different centers on the time scale over the entire period.
...
def plot_series_correct(anomalies, retailpriceseries,semi_labels,correct_pred,anomaly_list):
  ...
  for i in range(0,len(anomaly_list)):
    ...
    if(anomaly_list[i][1] in correct_pred[anomaly_list[i][0]] and (semi_labels[i] == 1 or semi_labels[i]==0)):
      ...
      if(count == 0):
        plt.fill_between(xaxis_region,yaxis_region, color ='r',label='Predicted:Weather, Actual:Hoarding')
      ...
      if(count2 ==0):
        plt.fill_between(xaxis_region,yaxis_region, color ='black',label='Predicted:Weather, Actual:Hoarding')
...
def train_test_function(align_m,align_d,align_l,align_b,data_m,data_d,data_l,data_b):
  ...
  delhi_semilabelsnew = newsemilabels(anomaliesdelhi,"data/anomaly/step2_labels_delhi.csv")
  lucknow_semilabelsnew = newsemilabels(anomalieslucknow,"data/anomaly/step2_labels_lucknow.csv")
  mumbai_semilabelsnew = newsemilabels(anomaliesmumbai,"data/anomaly/step2_labels_mumbai.csv")
  bangalore_semilabelsnew = newsemilabels(anomaliesbangalore,"data/anomaly/step2_labels_bangalore.csv")
  
  correct_predictions = load_correct_predictions("data/correct_predictions_step2.csv")
  ...
  plt.title('Delhi Anomalies Step 2 Result')
  plt.xlabel('Time')
