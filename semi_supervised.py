...
# Get derivative series of the input series
...
# Function to adjust the anomaly window
...
# Function to read the new curated labels in a usable format
...
# Function which takes in a list of anomalies and gives out a list of feature vectors corresponding to those anomalies
...
# function to partition the set of anomalies and put them in the cross validation set according to their period of occurrence
...
# Function which trains and predits on the test set
...
def train_test_function(align_m,align_d,align_l,align_b,data_m,data_d,data_l,data_b):
	# Read the file containing all types of normal periods
	...
	# Reading the new labels created by manually studying all the anomalies
	...
	# Get the feature vectors for all the anomalies
	...
	anomaly_id_new = []
	 ...
	 # Need to look carefully at this part of code while running the supervised or the semisupervised setting
	count = 0
	...
	# Create the cross validation sets using the partittion function
	...
	actual_labels = []
	test_anomaly_id = []
	# For loop which trains all sets except one and tests on the left out set
